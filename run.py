...
from data_reader import (
    Config, DataReader, DataReader_test, DataReader_pred, DataReader_mseed,
    DataReader_cont
)
from util import *
...
def read_args():
    ...
    parser.add_argument("--filters_root", default=8, type=int,
                        help="filters root")

    parser.add_argument("--depth", default=5, type=int, help="depth")

    parser.add_argument("--kernel_size", nargs="+", type=int, default=[7, 1],
                        help="kernel size")

    parser.add_argument("--pool_size", nargs="+", type=int, default=[4, 1],
                        help="pool size")

    parser.add_argument("--drop_rate", default=0, type=float,
                        help="drop out rate")

    parser.add_argument("--dilation_rate", nargs="+", type=int, default=[1, 1],
                        help="dilation rate")
    ...
    parser.add_argument("--class_weights", nargs="+", default=[1, 1, 1],
                        type=float, help="class weights")
    ...
    parser.add_argument("--tp_prob", default=0.3, type=float,
                        help="Probability threshold for P pick")

    parser.add_argument("--ts_prob", default=0.3, type=float,
                        help="Probability threshold for S pick")

    parser.add_argument("--input_length", default=None, type=int,
                        help="input length")

    parser.add_argument("--input_mseed", action="store_true",
                        help="mseed format")

    # For continuous data
    ############################################################################

    parser.add_argument("--input_mseed_cont", action="store_true",
                        help="continuous mseed format")

    parser.add_argument("--stations_csv", help="Path to csv inventory")

    parser.add_argument("--starttime", help="e.g. 2020-05-01 23:23:00")

    parser.add_argument("--endtime", help="e.g. 2020-05-01 23:23:00")

    ############################################################################

    parser.add_argument("--data_dir", default="./dataset/waveform_pred/",
                        help="Input file directory")

    parser.add_argument("--data_list", default="./dataset/waveform.csv",
                        help="Input csv file")

    parser.add_argument("--train_dir", default="./dataset/waveform_train/",
                        help="Input file directory")
...
def pred_fn(args, data_reader, figure_dir=None, result_dir=None, log_dir=None):
    ...
    with tf.Session(config=sess_config) as sess:
        ...
        if args.input_mseed or args.input_mseed_cont:
            ...
            while True:
                ...
                pred_batch, X_batch, fname_batch = sess.run(
                    [model.preds, batch[0], batch[1]],
            ...
            for step in tqdm(range(0, data_reader.num_data, args.batch_size),
                             desc="Pred"):
                if step + args.batch_size >= data_reader.num_data:
                    for t in threads:
                        t.join()
                ...
                picks_batch = pool.map(partial(
                    postprocessing_thread,
                    pred = pred_batch, X = X_batch,
                    fname = fname_batch,
                    result_dir = result_dir,
                    figure_dir = figure_dir,
                    args=args
...
def main(args):
    ...
    if args.mode == "train":
            ...
            if args.valid_list is not None:
                ...
                logging.info(
                    "Dataset size: train {}, valid {}".format(
                        data_reader.num_data, data_reader_valid.num_data))
            else:
        ...
        with tf.name_scope('create_inputs'):
            ...
            elif args.input_mseed_cont:
                stations = pd.read_csv(args.stations_csv, comment='/')
                stations = stations.code.tolist()

                data_reader = DataReader_cont(
                    stations=stations,
