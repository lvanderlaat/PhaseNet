...
PATH_FMT = '{data_dir}/i4.{station}.{channel}.{year}{julday:03d}_0+'
CHANNELS = ['HHE', 'HHN', 'HHZ']
FNAME_FMT = '_.{station}.{channels}.{timestamp}.{npts}.mseed'
...
class DataReader_mseed(DataReader):
    ...
    def __init__(self, data_dir, data_list, queue_size, coord,
                 input_length=3000, config=Config()):
        self.config = config
        tmp_list = pd.read_csv(data_list, header=0)
        self.data_list = tmp_list
        self.num_data = len(self.data_list)
    ...
    def add_placeholder(self):
        ...
        self.queue = tf.PaddingFIFOQueue(
            self.queue_size, ['float32', 'string'],
            shapes=[self.config.X_shape, []]
        )
    ...
    def read_mseed(self, fp, channels):
        ...
        data = np.hstack([data, np.zeros_like(data[:,:self.input_length//2]),
...
class DataReader_cont(DataReader):

    def __init__(self, stations, starttime, endtime, data_dir, queue_size, coord,
                 input_length=3000, config=Config()):
        self.config    = config
    ...
    def scan(self):
        ...
        juldays = range(self.starttime.julday, self.endtime.julday)
        ...
        for julday in juldays:
            for station in self.stations:
                ...
                for channel in CHANNELS:
                    fname = PATH_FMT.format(
                        data_dir=self.data_dir, station=station,
                        channel=channel, year=year, julday=julday
                    )
                    if os.path.exists(fname):
                        sta_fnames.append(fname)
                if len(sta_fnames) > 0:
                    self.fnames.append(sta_fnames)
        return
    ...
    def read_mseed(self, sta_fnames, channels=CHANNELS):

        meta = obspy.Stream()
        for sta_fname in sta_fnames:
            meta += obspy.read(sta_fname)
        ...
        timestamp = min([st.stats.starttime for st in meta]).timestamp
        npts = meta[0].stats.npts

        channels_out = []
        for component in ['E', 'N', 'Z']:
            try:
                channels_out.append(meta.select(component=component)[0].stats.channel)
            except:
                channels_out.append('-')

        channels_out = '_'.join(channels_out)

        station = meta[0].stats.station

        fname = FNAME_FMT.format(station=station, channels=channels_out,
                                 timestamp=timestamp, npts=npts)
    ...
    def thread_main(self, sess, n_threads=1, start=0):
        index = list(range(start, self.num_data, n_threads))
        for i in index:
            sta_fnames = self.fnames[i]
            try:
                meta, fname = self.read_mseed(sta_fnames)
            except Exception as e:
                logging.error("Failed reading {}".format(sta_fnames))
                print(e)
                continue
            for i in tqdm(range(meta.shape[0]), desc=f"{i}"):
                sample = meta[i]
                sample = self.normalize(sample)
                sample = self.adjust_missingchannels(sample)
                sess.run(
